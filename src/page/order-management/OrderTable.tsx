                ...
                <div className="mt-8 rounded-3xl bg-white p-5 shadow-sm">
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <input
                            ...
                            onChange={(e) => handleFilterChange(setSearchCustomer, e.target.value)}
                        ...
                        <input
                            value={searchFood}
                            onChange={(e) => handleFilterChange(setSearchFood, e.target.value)}
                                ...
                                {isFetching ? (
                                    ...
                                    ordersList.map((order: Order) => (
                                        <tr key={order.id} className="border-b hover:bg-gray-50/50">
                                            <td className="px-6 py-5 font-semibold">{order.order_number}</td>
                                            <td className="px-6 py-5">{order.user?.name || 'N/A'}</td>
                                            <td className="px-6 py-5">{order.items?.[0]?.food?.name || 'N/A'}</td>
                                            <td className="px-6 py-5">
                                                {order.created_at ? new Date(order.created_at).toLocaleDateString() : 'N/A'}
                                            </td>
                                            <td className="px-6 py-5">${order.total_amount}</td>
                                            <td className="px-6 py-5">
                                                <span className={`px-3 py-1 rounded-full text-xs font-medium capitalize ${getStatusStyle(order.status)}`}>
                                                    {order.status}
                                                </span>
                                            </td>
