        ...
        echo.private(channelName)
            .notification((notification: any) => {
                ...
                if (notification.type === 'new_order') {
                    ...
                    toast.error(notification.message || 'New private alert received', { icon: '🚫' });
                } else {
                ...
                <div className="rounded-3xl bg-white p-2 shadow-sm border border-gray-100 overflow-hidden">
                    {isFetching ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-3">
                            <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#207F36] border-t-transparent"></div>
                            <p className="text-sm font-medium text-gray-400">Loading incoming events data...</p>
                        </div>
                    ) : notificationsList.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <div className="text-4xl mb-3">📦</div>
                            <h3 className="text-base font-semibold text-gray-900">All caught up!</h3>
                            <p className="text-sm text-gray-400 mt-1">No alerts or notifications found right now.</p>
                        </div>
                    ) : (
                            ...
                            {notificationsList.map((notification) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`flex items-start gap-4 p-5 transition relative border-l-4 my-6 group ${isUnread
                                            ? 'border-l-[#207F36] cursor-pointer'
                                            : 'border-l-transparent opacity-75'
                                            } ${styles.bg}`}
                                    >
                                        {/* Status Icon */}
                                        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-lg shadow-sm ${styles.iconBg}`}>
                                            {styles.icon}
                                        </div>
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <div className="flex flex-wrap items-center gap-2 pr-12">
                                                <span className={`rounded-md px-2 py-0.5 text-xs font-semibold border ${styles.badge}`}>
                                                    {styles.label}
                                                </span>
                                            ...
                                            <div className="pt-1 flex items-center gap-4 text-xs text-gray-400">
                                                <span>Ref ID: <strong className="font-mono text-gray-600">{notification.data.order_number}</strong></span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3">
                                            <button
                                                ...
                                                className="p-2 text-gray-400 hover:text-rose-600 rounded-lg border border-transparent transition duration-150 cursor-pointer hover:bg-rose-50"
